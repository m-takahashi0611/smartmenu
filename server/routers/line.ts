...
interface IntentResult {
  ...
  mealType: string | null;  // 食事タイプ（朝食・昼食・夕飯など、会話文脈から推定）
}

async function classifyUserIntent(text: string, recentHistory: Array<{ role: string; content: string }> = []): Promise<IntentResult> {
  try {
    // 直近の会話履歴をシステムプロンプトの後に挿入（最大5件）
    const historyMessages = recentHistory.slice(-5).map(h => ({
      role: h.role as 'user' | 'assistant',
      content: h.content,
    }));
...
直前の会話履歴が提供される場合は、その文脈を踏まえて分類してください。
例：直前に「夕飯について話していた」文脈があれば、「おからを使いたい」「和風のお味噌汁を作りたい」などの発言も夕飯の献立リクエストとして分類できます。
...
- menu_vague: 献立について聴いている（曖昧・具体的問わず）。例：「夕飯どうしよう」「今日何作ろう」「献立迷ってる」「おからを使った夜ご飯を作りたい」「和風のお味噌汁を作りたい」「〇〇を使った料理を考えて」
- mood_theme: 今日の気分・食べたいもの・テーマを言っている（食材指定なし）。例：「今日は和食の気分」「さっぱりしたい」「カレーが食べたい」
...
重要：「〇〇を使った料理を考えて」「〇〇がある、何か作れる？」「〇〇を使って夜ご飯を作りたい」などの表現は menu_vague に分類する。ingredients_onlyと混同しないこと。
...
themeは気分・テーマの内容（「和食」「さっぱり系」「おから料理」「和風お味噌汁」など）、なければnull。menu_vagueで食材・料理テーマが含まれる場合もthemeに入れること。
mealTypeは会話の文脈や発言から推定できる食事タイプ。「夕飯」「夜ご飯」「晩御飯」「ディナー」→"dinner"、「朝食」「朝ごはん」→"breakfast"、「昼食」「ランチ」→"lunch"、明確でない場合はnull。
...
preferenceは好み内容（「人参嫌い」「ピーマン食べられない」など）、なければnull。`,
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: {
            ...
            required: ['intent', 'items', 'quantity', 'theme', 'memberName', 'preference', 'mealType'],
            additionalProperties: false,
          },
        },
      },
    ...
    return { intent: 'other', items: [], quantity: null, theme: null, memberName: null, preference: null, mealType: null };
  }
}
...
👇 下のボタンから選んでね！`, quickReply: { items: [
    ...
    case 'menu_vague': {
      // mealTypeが推定できている場合は確認なしで直接献立生成
...
献立を提案しましょうか？「献立」と送ってください`,
    ...
    // ※「献立」「冷蔵庫」等のキーワードに直接マッチする場合はLLM判定をスキップ（無限ループ防止）
    {
      ...
      const isDirectKeyword =
        ...
        // 「今日だけ特別」はリッチメニュー専用処理で扱うためAI分類をスキップ
        /^今日だけ特別/.test(text.trim());
...
👇 下のボタンから選んでね！`,
        ...
        // 直近の会話履歴を取得してLLMに渡す（文脈を踏まえた意図判定のため）
        const recentHistory = await getConversationHistory(lineUserId, 6).catch(() => []);
        // 現在のメッセージを除いた直近5件を渡す（現在のメッセージは別途userロールで渡される）
        const historyForClassify = recentHistory
          .filter(h => h.content !== text)
          .slice(-5)
          .map(h => ({ role: h.role, content: h.content }));
        const intentResult = await classifyUserIntent(text, historyForClassify);
