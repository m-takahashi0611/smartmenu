...
10. 【絶対禁止】返答の中で「冷蔵庫に〇〇があるので」「冷蔵庫の〇〇を使って」のように冷蔵庫の中身を列挙・言及してはいけない。冷蔵庫情報は内部参考用であり、返答には料理名・レシピのみを提案すること。
...
https://www.kondatebiyori.com`,
    ...
    const fridgeOverwriteMatch = text.match(/冷蔵庫(?:の中身)?(?:を全部消して|を全消しして|を全部削除して|を空にして|をリセットして)?(?:(.+?)(?:に書き換えて|に変えて|に入れ替えて|を入れて|を登録して|に更新して|に書き換えてください|に変えてください|に入れ替えてください|を入れてください|を登録してください|に更新してください))/);
